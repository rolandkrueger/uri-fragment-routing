// TODO
// - describe UriActionCommandFactory
//
= User Manual
...
In addition to the URI fragment, which is currently being interpreted, the routing context object is written to the log, too. By that, you have the option to include addtional information to the log by implementing the routing context's `toString()` method accordingly. You could, for example, log the current session ID or information about the remote host.
