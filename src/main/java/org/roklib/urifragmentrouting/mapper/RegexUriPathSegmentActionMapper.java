 ...
 * mapper will be responsible for handling this token, and hence the corresponding URI fragment path segment.
 * <p>
 ...
 * org.roklib.urifragmentrouting.UriActionMapperTree UriActionMapperTree}. Note that the first capturing group of a
 * {@link Matcher} for the current URI token will not be contained in the {@link StringListUriParameter}, since this
 * contains the entire match which corresponds to the matched URI token itself. <h1>Generating parameterized URI
 * fragments</h1> When you are generating parameterized URI fragments with {@link org.roklib.urifragmentrouting.UriActionMapperTree#assembleUriFragment(CapturedParameterValues,
 ...
 * ID is specified with the constructor. The concrete algorithm to assemble a valid URI token, which can be successfully
 ...
 * and the algorithm to assemble a URI token from a list of Strings which will later match the regex.
 */
public class RegexUriPathSegmentActionMapper extends DispatchingUriPathSegmentActionMapper {
     ...
     * through the {@link AbstractRegexToStringListParameterValueConverter} that has to be provided as the last
     * argument. This regex will be applied to the URI tokens passed in to {@link #isResponsibleForToken(String)} to
     * determine if this action mapper is responsible for handling the given token.
     *
     ...
     * so by checking whether the token matches the assigned regular expression. If that is the case {@code true}
     * is returned.
     *
     * @return {@code true} if the given URI token will be handled by this action handler, i.e. if the given URI token
     * matches the regular expression of the {@link AbstractRegexToStringListParameterValueConverter} specified in the
     * constructor.
     */
    @Override
     ...
     * specified with the constructor to create the concrete path segment name for this action mapper.
     *
